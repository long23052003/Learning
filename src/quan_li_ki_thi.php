    ...
    <main>
            ...
            echo "<div class='table-responsive'> <form action='' method='post'>";
                ...
                if (mysqli_num_rows($result) > 0) {
                    while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        echo '<td><input type="submit" class="btn_xoa btn-table" name="delete" value="Xóa"></td>';
                ...
                if (mysqli_num_rows($result) > 0) {
                    while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        echo "<td>" . $row['so_cau'] . "</td>";
                        echo "<td>" . $row['time'] . "</td>";
                        echo "<td><button class='btn btn-table' name='thi_ngay'><a href='ki_thi.php?id_ki_thi={$row['id_ki_thi']}&id_khoa_hoc={$row['id_khoa_hoc']}'>Thi ngay</a></button>
                        
                        <button class='btn btn-table' name='delete'><a href='xoa_ki_thi.php?id_ki_thi={$row['id_ki_thi']}'>Lịch sử</a></button>
                        </td>";
        ...
        if (isset($_POST['btn_submit'])) {
            if (empty($_POST['id_khoa_hoc']) || empty($_POST['ten_ki_thi']) || empty($_POST['so_cau_hoi']) || empty($_POST['thoi_gian'])) {
                echo "<div class='alert-danger' role='alert' style='width:100%;'>Vui long nhap day du thong tin</div>";
            } else {
                ...
                $sql = "INSERT INTO ki_thi (id_khoa_hoc, ten_ki_thi, so_cau, time, gioi_han) VALUES ($id_khoa_hoc, '$ten_ki_thi', $so_cau_hoi, $thoi_gian, $gioi_han)";
                $result = mysqli_query($conn, $sql);
                if ($result) {
