                ...
                <div>
                    ...
                    if (isset($_GET['id_khoa_hoc']) && $_GET['id_khoa_hoc'] != '' && isset($_GET['id_ki_thi']) && $_GET['id_ki_thi'] != '') {
                        if ($_SESSION['login']['role'] === 'user') {
                            $sql_check_gh = "SELECT * FROM diem_user WHERE id_user = {$_SESSION['login']['id']} AND id_khoa_hoc = $id_khoa_hoc AND id_ki_thi = $id_ki_thi";
                            $result_check_gh = mysqli_query($conn, $sql_check_gh);
                            if (mysqli_num_rows($result_check_gh) > $gioi_han) {
                                echo "bạn đã đến giới hạn";
                            } else {
